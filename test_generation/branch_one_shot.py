#!/usr/bin/env python
# the script to generate all test suites for branch coverage
import os
import sys
import random
import subprocess
...
for program in programs:
    ...
    # bei = count_branch - sum(1 for b in zip(*branch) if not any(b))
    for method in methods:
        if method == 'total':
            ...
            for test in selected_tests:
                print(test)
            output_path = os.path.join(test_suite_path, "branch_total_test_suite.txt")
            ...
            for test in selected_tests:
                print(test)
            output_path = os.path.join(test_suite_path, "branch_random_test_suite.txt")
            with open(output_path,'w') as f:
                for test in selected_tests:
                    f.write(test + '\n')
        ...
        if method == 'add':
            ...
            for test in selected_tests:
                print(test)
            output_path = os.path.join(test_suite_path, "branch_add_test_suite.txt")
            with open(output_path, 'w') as f:
                for test in selected_tests:
                    f.write(test + '\n')
