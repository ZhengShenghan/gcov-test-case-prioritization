#!/usr/bin/env python
# the script to generate all test suites for statement coverage
import os
import sys
import random
import subprocess
...
programs = ['tcas','schedule','schedule2','replace','printtokens','printtokens2','totinfo']
# programs = ['replace']
...
for program in programs:
    ...
    for line in output.splitlines():
        if "statementes executed" in line:
            num_statementes = line.split()[-1]
            print(f"Number of statementes: {num_statementes}")
            break
    ...
    # bei = count_statement - sum(1 for b in zip(*statement) if not any(b))
    for method in methods:
        if method == 'total':
            
            # Sort the tests based on the number of unique statementes they cover
            sorted_tests = sorted(tests, key=lambda test: len(test_coverage_map[test]), reverse=True)

            # Initialize the set to keep track of covered statementes
            covered_statementes = set()
            ...
            for test in sorted_tests:
                test_statementes = test_coverage_map[test]
                # Check if the test covers any new statementes
                if not test_statementes.issubset(covered_statementes):
                    # Add the test to the selected tests list
                    selected_tests.append(test)
                    # Update the set of covered statementes
                    covered_statementes.update(test_statementes)
        ...
        if method == 'add':
            ...
            # Iterate until all statementes are covered or all tests have been considered
            while covered_statementes != whole_statement_coverage and tests:
                # Select the test that covers the most additional statementes
                best_test = max(tests, key=lambda test: len(test_coverage_map[test] - covered_statementes))
                # Update the set of covered statementes and the list of selected tests
                covered_statementes.update(test_coverage_map[best_test])
